...
void putfont8(char *vram, int xsize, int x, int y, char c, char *font)
{
    ...
    for (i = 0; i < 16; i++){
        p = vram + (y + i) * xsize + x;
        d = font[i];
        if ((d & 0x80) != 0){p[0] = c;} // 1000 0000
        if ((d & 0x40) != 0){p[1] = c;} // 0100 0000
        if ((d & 0x20) != 0){p[2] = c;} // 0010 0000
        if ((d & 0x10) != 0){p[3] = c;} // 0001 0000
        if ((d & 0x08) != 0){p[4] = c;} // 0000 1000
        if ((d & 0x04) != 0){p[5] = c;} // 0000 0100
        if ((d & 0x02) != 0){p[6] = c;} // 0000 0010
        if ((d & 0x01) != 0){p[7] = c;} // 0000 0001
    }
    return;
}
